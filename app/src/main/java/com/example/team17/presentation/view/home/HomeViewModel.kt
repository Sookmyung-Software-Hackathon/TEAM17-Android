...
class HomeViewModel(private val homeRepository: HomeRepository): ViewModel() {
    ...
    fun initLocalBoggleList() {
        val local: MutableList<MyBoggleInfo> = mutableListOf(MyBoggleInfo("1", "2022년 08월 27일 13시", "테스트1", 6, 5))
    }

    fun getMyBoggleList() = viewModelScope.launch {
        kotlin.runCatching { homeRepository.getMyBoggle() }
            .onSuccess {
                _myBoggleList.value = it
            }
            .onFailure {
                it.printStackTrace()
                Timber.tag("Home-server").d("fail")
            }
    }
}